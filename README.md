...
- **Drink Menu**:
    - Espresso: Concentrated coffee (requires water and coffee)
...
```shell
git clone https://github.com/FrontN/Coffee-Machine.git
cd Coffee-Machine
```
...
2. Choose a drink from the menu (espresso/latte/cappuccino)

3. Type `report` at any time to see the machine's internal status

4. If resources are available, follow the prompts to insert coins

5. Collect your drink and any remaining change
...
## Machine Configuration ⚙️

You can easily adjust machine settings by modifying constants in the relevant files:

```python
WATER_ML = 300          # Water capacity per drink
MILK_ML = 200           # Milk capacity per drink
COFFEE_G = 18           # Coffee beans per drink
DRINK_COST = 2.5        # Base drink cost
```
...
- **State Management**: Maintains machine state across multiple transactions
